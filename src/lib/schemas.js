...
export const createReportSchema = z.object({
	suggestion: z
		...
		.min(1, { message: 'suggestion is required' })
		.trim(),
	area: z
		.string({ required_error: 'area is required' })
		.min(1, { message: 'area is required' })
		.max(64, { message: 'area must be 64 characters or less' })
		.trim(),
	status: z.enum(['OPENNED', 'ON_PROGRESS', 'CLOSED', 'NO_ACTION']),
	level: z.string(),
	caseId: z.string(),
	owner: z.string(),
	beforeImg: z
	...
	requestor: z.string({ required_error: 'User is required.' })
});

export const updateReportSchema = createReportSchema.omit({ caseId: true, requestor:true });